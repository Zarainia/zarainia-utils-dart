...
/// always clip splashes, see [FakeInkWell].
///
/// An [FakeInkResponse] widget does two things when responding to a tap:
///
...
///    grows.
///
/// The following two diagrams show how [FakeInkResponse] looks when tapped if the
/// [highlightShape] is [BoxShape.circle] (the default) and [containedInkWell]
/// is false (also the default).
///
/// The first diagram shows how it looks if the [FakeInkResponse] is relatively
/// large:
///
...
///
/// The second diagram shows how it looks if the [FakeInkResponse] is small:
///
...
///
/// The following diagram shows the effect when the [FakeInkResponse] has a
/// [highlightShape] of [BoxShape.rectangle] with [containedInkWell] set to
/// true. These are the values used by [FakeInkWell].
///
...
///
/// The [FakeInkResponse] widget must have a [Material] widget as an ancestor. The
...
/// [DecoratedBox], between the [Material] widget and the [FakeInkResponse] widget,
...
/// [Material] above the opaque widget but below the [FakeInkResponse] (as an
...
///  * [IconButton], which combines [FakeInkResponse] with an [Icon].
class FakeInkResponse extends StatelessWidget {
  ...
  /// [FakeInkResponse] or not. If [containedInkWell] is true, the splash remains
  /// centered around the tap location. If it is false, the splash migrates to
  /// the center of the [FakeInkResponse] as it grows.
  ///
  /// See also:
  ///
  ...
  /// [FakeInkResponse]. If the shape is [BoxShape.rectangle], then the highlight
  /// fills the [FakeInkResponse], or the rectangle provided by [getRectCallback] if
  ...
  /// specialize [FakeInkResponse] for unusual cases. For example,
...
class _InkResponseState extends State<_InkResponseStateWidget> with AutomaticKeepAliveClientMixin<_InkResponseStateWidget> implements _ParentInkResponseState {
  ...
  InteractiveInkFeature _createInkFeature(Offset globalPosition) {
    ...
    splash = (widget.splashFactory ?? Theme.of(context).splashFactory as InteractiveInkFeatureFactory).create(
      ...
      child: Actions(
        ...
        child: Focus(
          ...
          child: MouseRegion(
            ...
            child: Semantics(
              onTap: _simulateTap,
              onLongPress: _simulateLongPress,
...
///
/// For a variant of this widget that does not clip splashes, see [FakeInkResponse].
///
/// The following diagram shows how an [FakeInkWell] looks when tapped, when using
/// default values.
///
/// ![The highlight is a rectangle the size of the box.](https://flutter.github.io/assets-for-api-docs/assets/material/ink_well.png)
///
/// The [FakeInkWell] widget must have a [Material] widget as an ancestor. The
/// [Material] widget is where the ink reactions are actually painted. This
/// matches the material design premise wherein the [Material] is what is
/// actually reacting to touches by spreading ink.
///
...
///
/// If there is an opaque graphic, e.g. painted using a [Container], [Image], or
/// [DecoratedBox], between the [Material] widget and the [FakeInkWell] widget, then
...
/// If this is not possible for some reason, e.g. because you are using an
/// opaque [CustomPaint] widget, alternatively consider using a second
/// [Material] above the opaque widget but below the [FakeInkWell] (as an
...
///  * [GestureDetector], for listening for gestures without ink splashes.
///  * [ElevatedButton] and [TextButton], two kinds of buttons in material design.
///  * [FakeInkResponse], a variant of [FakeInkWell] that doesn't force a rectangular
///    shape on the ink reaction.
class FakeInkWell extends FakeInkResponse {
  /// Creates an ink well.
  ///
  /// Must have an ancestor [Material] widget in which to cause ink reactions.
  ///
